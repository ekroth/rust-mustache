...
impl<'a> rustc_serialize::Encoder<Error> for Encoder<'a> {
    ...
    fn emit_enum<F>(&mut self, _name: &str, _f: F) -> EncoderResult where
        F: FnOnce(&mut Encoder<'a>) -> EncoderResult,
    {
        Err(Error::UnsupportedType)
    }

    fn emit_enum_variant<F>(&mut self, _name: &str, _id: uint, _len: uint, _f: F) -> EncoderResult where
        F: FnOnce(&mut Encoder<'a>) -> EncoderResult,
    {
        Err(Error::UnsupportedType)
    }

    fn emit_enum_variant_arg<F>(&mut self, _a_idx: uint, _f: F) -> EncoderResult where
        F: FnOnce(&mut Encoder<'a>) -> EncoderResult,
    {
        Err(Error::UnsupportedType)
    }

    fn emit_enum_struct_variant<F>(&mut self, _v_name: &str, _v_id: uint, _len: uint, _f: F) -> EncoderResult where
        F: FnOnce(&mut Encoder<'a>) -> EncoderResult,
    {
        Err(Error::UnsupportedType)
    }

    fn emit_enum_struct_variant_field<F>(&mut self, _f_name: &str, _f_idx: uint, _f: F) -> EncoderResult where
        F: FnOnce(&mut Encoder<'a>) -> EncoderResult,
    {
        Err(Error::UnsupportedType)
    }

    fn emit_struct<F>(&mut self, _name: &str, _len: uint, f: F) -> EncoderResult where
        F: FnOnce(&mut Encoder<'a>) -> EncoderResult,
    {
    ...
    fn emit_struct_field<F>(&mut self, name: &str, _idx: uint, f: F) -> EncoderResult where
        F: FnOnce(&mut Encoder<'a>) -> EncoderResult,
    {
    ...
    fn emit_tuple<F>(&mut self, len: uint, f: F) -> EncoderResult where
        F: FnOnce(&mut Encoder<'a>) -> EncoderResult,
    {
        self.emit_seq(len, f)
    }

    fn emit_tuple_arg<F>(&mut self, idx: uint, f: F) -> EncoderResult where
        F: FnOnce(&mut Encoder<'a>) -> EncoderResult,
    {
        self.emit_seq_elt(idx, f)
    }

    fn emit_tuple_struct<F>(&mut self, _name: &str, len: uint, f: F) -> EncoderResult where
        F: FnOnce(&mut Encoder<'a>) -> EncoderResult,
    {
        self.emit_seq(len, f)
    }

    fn emit_tuple_struct_arg<F>(&mut self, idx: uint, f: F) -> EncoderResult where
        F: FnOnce(&mut Encoder<'a>) -> EncoderResult,
    {
        self.emit_seq_elt(idx, f)
    }
    ...
    // Specialized types:
    fn emit_option<F>(&mut self, _f: F) -> EncoderResult where
        F: FnOnce(&mut Encoder<'a>) -> EncoderResult,
    {
        Err(Error::UnsupportedType)
    }
    ...
    fn emit_option_some<F>(&mut self, _f: F) -> EncoderResult where
        F: FnOnce(&mut Encoder<'a>) -> EncoderResult,
    {
        Err(Error::UnsupportedType)
    }

    fn emit_seq<F>(&mut self, _len: uint, f: F) -> EncoderResult where
        F: FnOnce(&mut Encoder<'a>) -> EncoderResult,
    {
    ...
    fn emit_seq_elt<F>(&mut self, _idx: uint, f: F) -> EncoderResult where
        F: FnOnce(&mut Encoder<'a>) -> EncoderResult,
    {
    ...
    fn emit_map<F>(&mut self, _len: uint, f: F) -> EncoderResult where
        F: FnOnce(&mut Encoder<'a>) -> EncoderResult,
    {
        self.data.push(Data::Map(HashMap::new()));
        f(self)
    ...
    fn emit_map_elt_key<F>(&mut self, _idx: uint, mut f: F) -> EncoderResult where
        F: FnMut(&mut Encoder<'a>) -> EncoderResult,
    {
        try!(f(self));
    ...
    fn emit_map_elt_val<F>(&mut self, _idx: uint, f: F) -> EncoderResult where
        F: FnOnce(&mut Encoder<'a>) -> EncoderResult,
    {
